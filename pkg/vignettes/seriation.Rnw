...
Gruvaeus and Wainer & \code{"GW"} & Hamiltonian path length (restricted) & \code{dist} \\
Bond Energy Algorithm & \code{"BEA"} &
    Measure of effectiveness & \code{matrix} \\
TSP to optimize ME & \code{"BEA\_TSP"} &
    Measure of effectiveness& \code{matrix} \\
MDS -- first dimension & \code{"MDS"} & Least square crit.& \code{dist} \\
First principal component & \code{"PCA"} & 
    Least square crit.& \code{matrix} \\
Spectral seriation & \code{"Spectral"} & Inertia criterion & \code{dist} \\
