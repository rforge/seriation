...
following two gradient conditions hold~\citep{seriation:Hubert:1987}:
\begin{align}
...
layout(t(1:2))
#<<pimage2>>
## make all labels show
pimage(x, main = "Random", axes = 0)
text(1:dim(x)[2], par("usr")[3] - (par("usr")[4] - par("usr")[3]) / 20,
    srt = 45, adj = 1, labels =labels(x)[[2]], xpd = TRUE)
o_2mode <- c(o, ser_permutation(seq_len(ncol(x))))
pimage(x, o_2mode, main = "Reordered", axes = 0)
text(1:dim(x)[2], par("usr")[3] - (par("usr")[4] - par("usr")[3]) / 20,
    srt = 45, adj = 1, labels =labels(permute(x, o_2mode))[[2]], xpd = TRUE)
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth, trim=0 30 0 0]{seriation-pimage3}
...
If you have implemened a new criterion or seriation method, please consider
...
<<echo=FALSE, fig=FALSE, include=FALSE>>=
#$
bitmap(file = "seriation-ruspini.png", type = "pnggray", 
    height = 8, width = 8, res = 300, pointsize=12)
