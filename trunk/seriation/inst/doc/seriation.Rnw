...
found there. The idea was that different objects continuously go in and out of
...
only for the later row and column permutations are separable and can be
...
\end{displaymath}
\marginpar{check formulas}
BEA is in fact a simple suboptimal TSP heuristic using this distances
and instead of BEA any TSP solver can be used to obtain a order.
...
\centering
    \begin{tabular}{lccc}
        ...
        TSP to optimize bond energy & \code{"BEA\_TSP"} & \code{matrix} \\
        \hline
    \end{tabular}
