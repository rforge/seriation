...
\abstract{Unidimensional
seriation, i.e., finding a linear order for a set of objects given data, is a
basic problem in data analysis.  In this paper we present the
package~\pkg{seriation} which provides an infrastructure for unidimensional
seriation for R. This infrastructure comprises data structures to represent the
linear order, a wide array of seriation methods, quality criteria, and several
...
the most liberal to the most conservative according to their rulings,
psychologists may try to order subjects by their developmental stage using
questionnaire data, or archaeologists may order graves according to age given
the found artifacts. This type of problem is usually referred to as
\emph{unidimensional seriation.}
...
found there. The idea was that different objects continuously come into and 
go out of
fashion and a contingency table of grave sites times objects can be rearranged
using row and column permutations till all large values are close to the
diagonal. Initially, the rearrangement of rows and columns of the contingency
table was done manually and the adequacy was only judged subjectively by the
researcher. Later, \cite{seriation:Robinson:1951},
...
seriation criteria and methods. In Section~\ref{sec:infrastructure} we present
...
diagonal, the closer objects are together in the order of the matrix, the
higher their proximity. This provides a natural way to define consistency
between the proximity matrix and the order and results in the objective of
seriation: find the permutation $\pi^*$ which results in a rearranged proximity
matrix $\mathbf{S'} = (s_{\pi(i)\pi(j)})$ which is in Robinson form. However,
using this objective poses two problems:
\begin{enumerate}
...
The first problem can be addressed by relaxing the objective to finding a
permutation which results into a matrix as close to the Robinson form as
possible. To quantify closeness, measures of divergence from the Robinson form
have to be defined. Such measures together with other quality measures not
directly related to the Robinson form but with a similar goal are defined in
Section~\ref{sec:criteria}.

The second problem, to find the permutation that minimized the measure of
divergence from the Robinson form or other quality measures can be addressed by
using search heuristics or methods which reduce the search space 
\citep[e.g., for dynamic programming see][]{seriation:Hubert:1981}. 
Search heuristics which improve different quality measures are presented in
...
push large values away from it. A measure to quantify this is
...
The criterion clearly increases when higher dissimilarity values are placed
...
each node is visited exactly once, i.e., a Hamilton path. The length of such a
path is given by
\begin{equation}
...
\label{sec:methods}
In the literature many methods for seriation have been proposed. In this
...
\subsection{Principle components analysis and correspondence analysis}
\marginpar{PCA, CA,\dots}
...
\begin{itemize}
    \item \code{Order\_matrix} stores two (integer) permutation vectors, one
        ...
        implimented as a list with the two elements \code{row} and
        \code{column}.
    \item \code{Order\_vector} stores a single permutation vector to reorder
        ...
        is not implemeted as just a vector, but as a list with the element
...
the comparision several seriation methods for dissimilarity matrices described
in Section~\ref{sec:methods}. 

<<>>=
methods <- c("TSP", "Chen", "MDS", "HC", "GW", "OLO")
...
To produce a heat map with optimally reordered dendrogramms, the function
