...
of loss and merit functions, and several visualization techniques which build
on seriation. To illustrate how easily the package can be used for a variety of
applications, a comprehensive collection of examples is presented.}
...
analysis require the solution of discrete optimization problems which involve
evaluating all feasible solutions. Due to the combinatorial nature, the number
of possible solutions grows with problem size (number of objects) by the order
$O(n!)$. This makes a brute-force enumerative approach infeasible for all but
very small problems.  To solve larger problems, partial enumeration methods can
be used.  For example, \cite{seriation:Hubert:1987} propose dynamic programming
and \cite{seriation:Brusco:2005} use branch-and-bound. For even larger problems
only heuristics can be used.
...
to find a chronological order for graves discovered in the Nile area given
objects found there. The idea was that different objects continuously come into
and go out of fashion and a contingency table of grave sites times objects can
be rearranged using row and column permutations till all large values are close
to the diagonal.  Initially, the rearrangement of rows and columns of the
...
\cite{seriation:Kendall:1971} and others proposed measures of agreement between
rows to quantify optimality of the resulting table. A comprehensive description
of the development of seriation in archeology is presented by
\cite{seriation:Ihm:2005}.

%In ecology,  
%R Software for ecology: \pkg{ade4}, \pkg{vegan}, \pkg{cocorresp}
%\marginpar{block seriation missing}

\marginpar{fix: In this paper\dots}
In this paper we concentrate on seriation as a combinatorial optimization 
task and we present the package~\pkg{seriation} which provides the
...
In Section~\ref{sec:methods} we give an overview of
 seriation methods. 
...
We define a permutation function $\Psi$ as a function which
reorders the objects in $\mathbf{D}$ by
simultaneously permuting rows and columns.
The seriation problem is to find a permutation function $\Psi$
...
which
optimizes the value of a given loss function $Q$ or merit function $M$. 
This results in the
...
A symmetric dissimilarity matrix is known as $2$-way $1$-mode data since it has
columns and rows ($2$-way) but only represents one set of objects ($1$-mode).
Seriation is also possible for $2$-way $2$-mode data which are represented by a
general positive matrix. In such data columns and rows represent two sets of
objects which are both brought into a linear order. For loss functions for
$2$-way $2$-mode data the optimal order of columns can be dependent of the
order of rows and vice versa or it can be independent allowing for breaking the
optimization down into two separate problems, one for the columns and one for
the rows. Another way to deal with the seriation for $2$-way $2$-mode data is
to calculate two dissimilarity matrices, one for each mode, and then solve two
seriation problem on $2$-way $1$-mode data.  
Furthermore, seriation can be generalized to $k$-way
$k$-mode for data in the form of a $k$ dimensional array by defining suitable
loss functions for such data or by breaking the problem down into several lower
dimensional independent problems which can be solved.


In
the following subsections, we review some commonly used loss functions.  Most
functions are used for $2$-way $1$-mode data but the measure of effectiveness
and stress can be also used for $2$-way $2$-mode data. 
...
\begin{align}
    \text{rows:} & \quad  d_{ik} \le d_{ij} \quad \text{for} 
        \quad 1 \ge i < k < j \ge n; \\ 
        \text{columns:} & \quad  d_{kj} \ge d_{ij} \quad \text{for} 
        \quad 1 \ge i < k < j \ge n.
\end{align}

In a anti-Robinson matrix the smallest dissimilarity values appear close to
the main diagonal, the closer objects are together in the order of the matrix,
the higher their similarity. This provides a natural objective for seriation. 

It has to be noted that $\mathbf{D}$ can be brought by row and column
permutation into a perfect anti-Robinson form whenever $\mathbf{D}$  is an
...
\begin{equation}
    M(\mathbf{D}) = 
    \sum_{i<k<j}f(d_{ij}, d_{ik}) + \sum_{i<k<j}f(d_{ij}, d_{kj}) 
    \label{equ:gradient}
\end{equation}
where $f(\cdot,\cdot)$ is a function which defines how a violation
or satisfaction 
of a gradient condition for an object triple ($O_i$, $O_k$ and $O_j$)
is counted.

\cite{seriation:Hubert:1987} suggest two functions. The first
...
An even simpler loss function can be created
in the same way as the gradient measures above by only concentrating
on violations. 
\begin{equation}
    Q(\mathbf{D}) = 
...
\begin{equation}
    f(z, y) = |z-y|I(z, y)
\end{equation}
using the absolute deviations as the weight.
...
ME was developed for $2$-way $2$-mode data, however, ME can also be used for a
symmetric  matrix ($1$-mode data) and gets maximal only if all large values are
grouped together around the main diagonal. 


Note that from the definition in equation~(\ref{equ:ME}) it can be seen that the
contribution of column and row order to the loss function is additive and thus
independent. 

\subsection{Stress}
\label{sec:stress}

Stress measures the conciseness of the presentation of a matrix ($2$-mode data) 
and can
...
symmetric proximity matrices ($1$-mode data).
%, 
...
Solving the discrete optimization problem for seriation 
with most loss/merit functions is
clearly very hard, since the number of
possible permutations is $n!$ which makes an exhaustive
search for sets with a medium to large number of objects infeasible.
In this section, we describe some methods
(partial enumeration, heuristics and other methods)
which are typically used for seriation. For each method we state for
which type of loss functions it is suitable and if it finds the optimum or is
a heuristic.
...
storing $2^n-1$ results. Although $2^n-1$ grows at a
lower rate than $n!$ and is
for $n \gg 3$ considerably smaller,
the storage requirements of $2^n-1$ results still grow fast, limiting the
maximal problem size severely. For example, for $n=30$ more than one billion
results have to be calculated and stored, clearly a number to large for the
main memory capacity of most current computers.
...
reduction of search space is strongely data dependent and poorly structured
data can lead to very poor performance.
With branch-and-bound slightly larger problems can be solved than with
dynamic programming in reasonable time.
\cite{seriation:Brusco:2005} state that depending on the data, in some cases
proximity matrices with 40 or more objects can be handled.

Partial enumeration methods can be used 
to find the exact solution independently of the loss function. However, 
partial enumeration is limited to only relatively small problems.
...
As the general seriation problem, solving the TSP is difficult. 
In the seriation case with $n+1$
...
\citep{seriation:Held:1962} and larger instances of several hundred
objects can be solved using
\emph{branch-and-cut} algorithms~\citep{seriation:Padberg:1990}. For even large
...
heuristics and exact methods can be found in \cite{seriation:Gutin:2002}.
...
Although hierarchical clustering solves a optimization problem different to
...
\begin{itemize}
    \item for $2$-way $1$-mode data the class \code{dist},
    \item for $2$-way $2$-mode data the class \code{matrix}, and
...
\begin{figure}[tp]
    ...
    \caption{Data structures for permutations provided by \pkg{seriation}}
    \label{fig:infrastructure}
\end{figure}

However, to represent the permutation vector 
for each mode no data structures are
available. \pkg{seriation} provides the necessary data structure (using the S3
class system) depicted in Figure~\ref{fig:infrastructure}. The class
\code{ser\_permutation} represents the a permutation information for $k$-mode
data (including the cases of $k=1$ and $k=2$). It consists of $k$ permutation
vectors, where each permutation vector is represented by an concrete
implementation of the abstract class \code{ser\_permutation\_vector}. 
Currently, the
permutation vector can be stored as a simple integer vector or as an object of
class \code{hclust} (an object describing a hierarchical clustering tree
including an ordering for the trees node leaves defined in \pkg{stats}).
...
For \code{ser\_permutation} also a constructor is provided which can bind
$k$ \code{ser\_permutation\_vector} 
objects together into an object for
$k$-mode data. \code{ser\_permutation} is implemented as a list of length $k$
and each element contains a \code{ser\_permutation\_vector} object.  
Methods like
\func{length}, accessing elements with \code{"[["}, \code{"[[<-"}, 
subsetting with \code{"["},
and combining  with \func{c} work as expected.  Also a
\func{print} method is provided. Finally, direct access to the raw permutation
vectors is available using \func{get\_order}. 
Here the second argument (which defaults to $1$) specifies
the dimension (mode) for which the order vector is requested.
...
\end{center}
where \code{x} is the input data, \code{method} is a string defining
the seriation method to be used and \code{control} can contain a list 
with additional information for the algorithm.
\func{seriation} returns an object of class \code{ser\_permutation}
of length conforming to the number of dimensions of \code{x}.

\marginpar{write about the methods.}

For \code{matrix} the additional argument
...
\centering
    \begin{tabular}{lllc}
        ...
        \hline
        Branch-and-bound & \code{"BBURCG"} & 
Gradient measure &\code{dist} \\
        Branch-and-bound & \code{"BBWRCG"} & 
Gradient measure (weighted)& \code{dist} \\
        Simulated annealing & \code{"ARSA"} & 
Gradient measure &\code{dist} \\
        TSP solver& \code{"TSP"} &
Hamiltonian path length& \code{dist} \\
...
Restricted Hamiltonian path length& \code{dist} \\
        Rank-two ellipse seriation & \code{"Chen"} & 
Other& \code{dist} \\
        MDS (first dimension) & \code{"MDS"} &
Other& \code{dist} \\
        First principal component  & \code{"PCA"} & 
Other& \code{matrix} \\
        \hline
    \end{tabular}
\caption{Currently implemented seriation methods.}
\label{tab:methods}
\end{table}
...
methods currently available in the package for seriation. Over time more
...
performed) and \code{method} specifies the type of loss function. A vector of
several methods can be used resulting in a named vector.  We already defined
different loss/merit functions for seriation 
in Section~\ref{sec:seriation}.  In
Table~\ref{tab:criteria} we indicate 
the loss/merit functions currently available in
the package.  Additionally, the pseudo method \code{"all"} (which is the
default) can be used which calculated the values for all applicable 
loss/merit functions.
...
$1$-mode data. The permutation vector length is $150$ for the
...
To use the result of the seriation for the original $2$-mode data,
