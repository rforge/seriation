...
given available data and some loss or merit function.  Together with
...
larger problems only heuristics can be used.

It has to be noted that seriation has a rich history in archeology.
\cite{seriation:Petrie:1899} was the first to use seriation as a formal
method.  He applied it to find a chronological order for graves
discovered in the Nile area given objects found there.  The idea was
that different objects continuously come into and go out of fashion and
a contingency table of grave sites times objects can be rearranged using
row and column permutations till all large values are close to the
diagonal.  Initially, the rearrangement of rows and columns of the
...
Techniques related to seriation are also popular in several other fields.
Especially in ecology scaling techniques are used under the name
\emph{ordination}. For these applications several R packages already exist
(e.g. \pkg{ade4}~\cite{seriation:Chessel:2007} and 
\pkg{vegan}~\cite{seriation:Oksanen:2007}). This paper describes the new
package \pkg{seriation} which differs from existing packages in the following
way.
\begin{enumerate}
...
The paper starts with a formal introduction of the seriation problem as a
combinatorial optimization problem in Section~\ref{sec:seriation}.  In
Section~\ref{sec:methods} we give an overview of seriation methods.  In
Section~\ref{sec:infrastructure} we present the infrastructure provided by the
package~\pkg{seriation}.  In Section~\ref{sec:example} we present several
examples and applications for seriation. We conclude with
...
\end{equation}
It weighs each satisfaction or violation by the difference by its
...
Hamiltonian path length results in an seriation optimal with respect to
...
by heat maps to reorder rows and columns with the aim to place more similar
...
be defined.  \cite{seriation:Gruvaeus:1972} suggest to obtain a unique order by
...
order <- sapply(methods, FUN = function(m) seriate(d, m), simplify = FALSE)
@
...
def.par <- par(no.readonly = TRUE)
layout(matrix(seq_len(6), ncol = 3, byrow = TRUE))
<<pimage3>>
par(def.par)
tmp <- dev.off()
@
...
Based on seriation, several visualization techniques are provides by
\pkg{seriation}. Especially the optimally reordered heat map, the bertin plot
and the dissimilarity plot have to be mentioned since they present clear
improvements over standard plots.

Future work on \pkg{seriation} will focus on adding different seriation methods.
For example, by adding methods for block seriation which aim to finding
simultaneous partitions of rows and columns in a data matrix~\citep[see,
