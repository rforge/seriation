...
\begin{figure}
    \centering
    \includegraphics{seriation-pimage3}
...
\begin{displaymath}
  \phi = \Phi(\mathbf{X})  
\end{displaymath}
is defined. Given $\Pi$, the set of all row and column permutations
$\pi$ of matrix $\mathbf{X}$, the optimal permutation with respect to
purity
\begin{displaymath}
...
<<>>=
order <- Order(
    row = seriate(dist(orig_matrix, "minkowski", p = 1), method=  "mds"),
    col = seriate(dist(t(orig_matrix), "minkowski", p = 1), method = "mds")
)
@
...
Optimally reordered heat map.
<<fig=TRUE>>=
hmap(orig_matrix, distfun=function(x) dist(x, "minkowski", p = 1))
@

<<fig=TRUE>>=
hmap(orig_matrix, dendrogram = FALSE, 
    distfun=function(x) dist(x, "minkowski", p = 1))
@
...
<<fig=TRUE>>=
order <- seriate(Townships, method = "bea")
