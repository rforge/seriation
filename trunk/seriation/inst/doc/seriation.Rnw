...
\marginpar{Absatz ueber Was wir nicht machen fehlt.}

In ecology,  
R Software for ecology: \pkg{ade4}, \pkg{vegan}, \pkg{cocorresp}
\marginpar{block seriation missing}
Also say something about multidimensional scaling and principal
...
\begin{align}
    ...
    \text{within columns:} & \quad  d_{kj} \le d_{ij} \quad \text{for} 
        \quad 1 \ge i < k < j \ge n.
\end{align}
\marginpar{In Equations 2 und 3 passen die richtungen der vergleichsoperatoren
nicht, sollen die $\ge$ alle $\le$ sein?}
