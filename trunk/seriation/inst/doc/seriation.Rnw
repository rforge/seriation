...
a cross-tabulation of grave sites and objects can be rearranged using
row and column permutations till all large values are close to the
diagonal.  Initially, the rearrangement of rows and columns of this
...
methods, such as for example methods for block seriation which aim to
