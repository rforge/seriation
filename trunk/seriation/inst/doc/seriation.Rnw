...
seriation problem.  Furthermore, seriation can be generalized to  $k$-way
...
\marginpar{is it a merit function?}
...
However, to represent the permutation for each mode no data structures are
available. \pkg{seriation} provides necessary data structure (using the S3
class system) depicted in Figure~\ref{fig:infrastructure}. The class
\code{ser\_permutations} represents the a permutation information for $k$-mode
...
implementation of the abstract class \code{ser\_permutation}. Currently, the
...
\code{ser\_permutation} has a constructor \func{permutation} which 
converts data into the correct concrete subclass of \code{ser\_permutation} and
checks if it contains a proper permutation vector.
For \code{ser\_permutation} the methods \func{print}, \func{length} for the
length of the permutation vector, \func{get\_method} to get the method used to
generate the permutation, and \func{get\_order} to access the raw (integer)
permutation vector are available.  The access method \func{get\_order} is
implemented for each concrete subclass of \code{ser\_permutation}. With this
design, other classes representing permutations can be easily incorporated by
just adding a appropriate \func{get\_order} methods for the new class.

For \code{ser\_permutations} also a constructor is provided which can bind
several ($k$) \code{ser\_permutation} objects together into an object for
$k$-mode data. \code{ser\_permutations} is implemented as a list of length $k$
and each element contains a \code{ser\_permutation} object.  Methods like
\func{length}, accessing elements with \code{"[["}, \code{"["} \code{"[[<-"}
and \code{"[<-"}, and combining  with \func{c} work as expected.  Also a
\func{print} method is provided. Finally, direct access to the raw permutation
vectors is available using \func{get\_order}. 
Here the second argument signifies
the dimension (mode) which defaults to $1$.

\marginpar{complete rewrite}

The seriation algorithms are available via the function \func{seriate} defined
as:
...
\func{seriation} returns an object of class \code{ser\_permutations}
of length conforming to \code{x}.
...
\centering
    \begin{tabular}{lccc}
        ...
        Gruvaeus and Wainer & \code{"GW"} &  \code{dist} \\
        Optimal leaf ordering & \code{"OLO"} &  \code{dist} \\
...
To calculate the value of the loss function for a certain permutation,
the method
...
object of class \code{ser\_permutations} (if omitted no permutation is
...
Table~\ref{tab:criteria} we indicate the loss functions currently available in
the package.  Additionally, the pseudo method \code{"all"} (which is the
default) can be used which calculated the values for all applicable criteria.
...
\code{ser\_permutations} object of suitable length. 
...
The result is an object of class \code{ser\_permutations} for 
...
we have to add a permutation vector to the \code{ser\_permutations}
...
order_2mode <- c(order, permutations(1:ncol(x)))
order_2mode
