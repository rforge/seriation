...
\citep{bga:Agrawal+Imielinski+Swami:1993,bga:Agrawal+Srikant:1994} are
...
\cite{bgb:Hastie+Tibshirani+Friedman:2001}, where questionnaires data
...
as a S4 class named \texttt{assMatrix} containing the following S4
classes:
\begin{itemize}
...
format. This allows the retransformation of a \texttt{assMatrix} into
...
\citep{bga:Borgelt+Kruse:2002,bga:Borgelt:2003}. The code is called
...
\texttt{assMatrix} (or a data format which can be coerced to
\texttt{assMatrix}). Furthermore, information on the parameters needs
...
programs \citep[cp.][]{bgm:Borgelt:2004} except that by default the original
...
\begin{itemize}
 \item default: \texttt{character}, one of ``head'', ``body'',
  ...
  apriori manual;][]{bgm:Borgelt:2004}
...
\texttt{assMatrix} data is used for determining the column number if
...
learning repository \citep{bgm:Blake+Merz:1998}. 
<<data>>=
library(arules)
data(census)
dim(census)
...
number of different items is \Sexpr{dim(census.assMatrix)[1]}.
