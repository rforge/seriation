/*----------------------------------------------------------------------
modefication from Bettina Gruen based on Christian Borgelt's code
----------------------------------------------------------------------*/
...
static const char* ttarget[] = {
  /* TT_SET      0 */  "frequent item sets",
  /* TT_MFSET    1 */  "maximally frequent item sets",
  /* TT_CLSET    2 */  "closed item sets",
  /* TT_RULE     3 */  "rules",
...
{                               /* --- read appearance indicators */
  ...
  for (i = 0; i < 4; i++) {
	  for (j = 0; j < set[i]; j++) {
		  ...
		  item->app = appcode_i(i);  /* get the appearance indicator */
...
SEXP returnObject(RULESET *set, ARparameter *param)
{
	...
	rnb = PROTECT(allocVector(INTSXP, 1));
 	INTEGER(rnb)[0] = set->rnb;
	SET_SLOT(ans, install("rnb"), duplicate(rnb));
	UNPROTECT(1);
	body = PROTECT(NEW_OBJECT(MAKE_CLASS("cscMatrix")));
	tp = PROTECT(allocVector(STRSXP, set->ttotal));
  	for (i = 0; i < set->ttotal; i++) SET_STRING_ELT(tp, i, mkChar(set->body[i]));
	SET_SLOT(body, install("i"), AS_INTEGER(tp));
	UNPROTECT(1);
	tp = PROTECT(allocVector(INTSXP, set->rnb+1));
	INTEGER(tp)[0] = 0;						     
	for (i = 0; i < set->rnb; i++) INTEGER(tp)[i+1] = set->tnb[i];
 	SET_SLOT(body, install("p"), duplicate(tp));
	UNPROTECT(1);
	tp = PROTECT(allocVector(REALSXP, set->ttotal));
	for (i = 0; i < set->ttotal; i++) REAL(tp)[i] = 1;
	SET_SLOT(body, install("x"), duplicate(tp));
	UNPROTECT(1);
	dim = PROTECT(allocVector(INTSXP, 2));
	INTEGER(dim)[0] = set->cnt;
	INTEGER(dim)[1] = set->rnb;
	SET_SLOT(body, install("Dim"), duplicate(dim));
	SET_SLOT(ans, install("body"), duplicate(body));
	...
	j = set->rnb;
	SET_STRING_ELT(names, 0, mkChar("support"));
	k = 1;	
	if (param->target > TT_CLSET) {
	...
	if (param->target == TT_RULE) {
		...
		head =  PROTECT(NEW_OBJECT(MAKE_CLASS("cscMatrix")));
		tp = PROTECT(allocVector(STRSXP, set->rnb));
		for (i = 0; i < set->rnb; i++) SET_STRING_ELT(tp, i, mkChar(set->head[i]));
		SET_SLOT(head, install("i"), AS_INTEGER(tp));
		UNPROTECT(1);
		tp = PROTECT(allocVector(INTSXP, set->rnb+1));
		for (i = 0; i < set->rnb+1; i++) INTEGER(tp)[i] = i;
		SET_SLOT(head, install("p"),  duplicate(tp));
		UNPROTECT(1);
		tp = PROTECT(allocVector(REALSXP, set->rnb));
		for (i = 0; i < set->rnb; i++) REAL(tp)[i] = 1;
		SET_SLOT(head, install("x"),  duplicate(tp));
		UNPROTECT(1);
		SET_SLOT(head, install("Dim"), duplicate(dim));
		SET_SLOT(ans, install("head"), duplicate(head));
		UNPROTECT(1);
	}
	UNPROTECT(2);
	if (param->aval) {
	...
	if (param->ext) {
		...
		j += set->rnb;
		if (param->target == TT_RULE) SET_STRING_ELT(names, k++, mkChar("body.support"));
		else SET_STRING_ELT(names, k++, mkChar("exact.support"));
	}
	...
	UNPROTECT(3);
	if (param->trans) {
		trans = PROTECT(NEW_OBJECT(MAKE_CLASS("cscMatrix")));
		...
		UNPROTECT(1);
		SET_SLOT(ans, install("trans"), duplicate(trans));
		UNPROTECT(1);
	}
	UNPROTECT(1);
	return ans;
