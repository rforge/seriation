setClass("ASapp",
         ...
         validity = function(object) {
           app <- c("i",  "in",  "a", "ante", "antecedent", "b", "body",
                    "o",  "out", "c", "cons", "consequent", "h", "head",
                    "io", "inout", "ac", "bh", "both",          
                    "n", "neither", "none", "ign", "ignore", "-")
           ans <- NULL
           if (!object@default %in% app) ans <- c(NULL, "Default value not specified correctly!")
...
setAs("list", "ECparms", function(from, to) .list2object(from, to))
setAs("list", "APcontrol", function(from, to) .list2object(from, to))
setAs("list", "APparms", function(from, to) .list2object(from, to))

###**********************************************************
setMethod("initialize", "ASparms",
