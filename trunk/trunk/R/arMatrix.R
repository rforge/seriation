...
setAs("arMatrix", "matrix", 
      function(from) {
        from <- as(from, "cscMatrix")
        as(from, "matrix")
      })

setAs("data.frame", "arMatrix", function(from) {
  ...
  new("arMatrix", z, attrib)
})

setAs("arMatrix", "data.frame", function(from) {
  variables <- from@attr
  to <- list()
  for (v in 1:length(variables)) {
    t <- .Call("arMatrix_var", from, as.integer(v), PACKAGE = "arules")
    var <- variables[v]
...
setMethod("as.character", signature(x = "arMatrix"), function(x) {
...
setMethod("show", signature(object = "arMatrix"), function(object) {
...
setMethod("%in%", signature(x = "arMatrix"), function(x, table) {
